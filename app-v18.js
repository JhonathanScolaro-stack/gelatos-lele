(() => {
  ...
  function settingsScreen() {
    ...
    if (section === 'cloud') {
      ...
      if (!signedIn) return '<section class="screen active">' + heading('Configurações', 'Nuvem e sincronização', 'Crie um acesso para que estoque, pedidos e financeiro fiquem iguais nos celulares da empresa.') + '<form id="cloudAuthForm" class="panel form-panel"><h2>Entrar ou criar acesso</h2>' + field('Seu e-mail', '<input name="email" type="email" autocomplete="email" required placeholder="voce@exemplo.com">') + field('Senha de acesso ao app', '<input name="password" type="password" autocomplete="current-password" minlength="8" required>', 'Crie uma senha somente para entrar no Gelatos Lele. Não é a senha do banco de dados.') + '<div class="button-row"><button class="primary" name="cloudMode" value="signin">Entrar</button><button class="outline" name="cloudMode" value="signup">Criar acesso</button></div><p class="form-note">Ao criar o acesso, confirme o e-mail se o Supabase solicitar e depois volte aqui para entrar.</p></form></section>';
