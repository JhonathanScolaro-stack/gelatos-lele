-- Gelatos Lele v47 — catálogo com URLs de mídia.
-- Execute este arquivo inteiro UMA vez no SQL Editor do projeto Supabase.
-- Ele não altera estoque, pedidos ou financeiro; só passa a publicar URLs
-- externas de fotos/logos quando elas já existirem no estado da empresa.
...
as $$
  with settings as (
    ...
    select coalesce(jsonb_agg(jsonb_build_object(
      ...
      'imageToken', case
        when coalesce(recipe.item->>'imageUrl', '') <> '' then md5(recipe.item->>'imageUrl')
        when coalesce(recipe.item->>'imageData', '') <> '' then md5(recipe.item->>'imageData')
        else ''
      end
  ...
  select jsonb_build_object(
    ...
    'logoToken', case
      when coalesce(nullif(settings.item->>'catalogLogoUrl', ''), nullif(settings.item->>'headerLogoUrl', '')) <> '' then md5(coalesce(nullif(settings.item->>'catalogLogoUrl', ''), nullif(settings.item->>'headerLogoUrl', '')))
      when coalesce(nullif(settings.item->>'catalogLogoDataUrl', ''), nullif(settings.item->>'headerLogoDataUrl', '')) <> '' then md5(coalesce(nullif(settings.item->>'catalogLogoDataUrl', ''), nullif(settings.item->>'headerLogoDataUrl', '')))
      else ''
    end,
...
-- Mantém as fotos antigas sob demanda até a migração terminar. URLs externas
-- nunca retornam pelo Supabase, portanto não criam novo egress nele.
...
begin
  ...
  from (select trim(value) as value from jsonb_array_elements_text(p_image_ids) as incoming(value) where length(trim(value)) between 1 and 120 limit 30) request;
  ...
  for v_recipe in select value from jsonb_array_elements(coalesce(v_data->'recipes', '[]'::jsonb)) loop
    if v_recipe->>'id' = any(v_image_ids) and coalesce((v_recipe->>'active')::boolean, true) and coalesce(v_recipe->>'imageData', '') <> '' then
